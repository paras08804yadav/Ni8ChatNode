...
const getFeed = async (req, res) => {
  ...
  try {
    ...
    // Map preferred and other hosts to the required format
    const formattedPreferredHosts = preferredHosts.map(host => ({
      host_id: host._id,
      hostname: host.hostname,
      gender: host.gender,
      location: host.location,
    ...
    const formattedOtherHosts = otherHosts.map(host => ({
      host_id: host._id,
      hostname: host.hostname,
      gender: host.gender,
      location: host.location,
    ...
    res.json({
      preferredHosts: formattedPreferredHosts,
      otherHosts: formattedOtherHosts,
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Failed to fetch hosts' });
