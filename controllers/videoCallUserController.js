...
const startVideoCall = async (req, res) => {
    ...
    try {
        // Step 2: Fetch user and host data to get the current balance and video call rate
        const user = await User.findById(user_id).select('wallet_balance');
        const host = await Host.findById(host_id).select('video_rate'); // Fetch the host's per-minute video call rate

        if (!user || !host) {
            return res.status(404).json({ msg: 'User or Host not found.' });
        }

        const videoRate = host.video_rate; // Host's per-minute coin rate for video calls

        // Step 3: Check if the user has enough coins for at least 1 minute of the video call
        if (user.wallet_balance < videoRate) {
            return res.status(400).json({ msg: 'Insufficient balance to start the video call.' });
        }

        // Step 4: Deduct coins for the first minute (even if call is ongoing, deduct in advance)
        const result = await updateCoins(user_id, host_id, 'video_call');

        if (!result.success) {
            return res.status(400).json({ msg: result.message });
        }

        // Step 5: Start the video call (Socket implementation or real-time logic can go here)
        // Placeholder for starting the call in real-time:
        console.log('Socket implementation: Video call started.');

        // Return success message and updated balance
        res.status(200).json({
            msg: 'Video call started successfully, coins deducted for the first minute.',
        ...
        const intervalId = setInterval(async () => {
            ...
            if (updatedUser.wallet_balance < videoRate) {
            ...
            const nextResult = await updateCoins(user_id, host_id, 'video_call');

            if (!nextResult.success) {
                // Handle any deduction failure
